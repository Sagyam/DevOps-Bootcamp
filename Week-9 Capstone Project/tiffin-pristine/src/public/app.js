...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderMenuCards(items) {
    if (!items || items.length === 0) {
      menuGrid.innerHTML =
        '<div class="text-center py-4 text-muted">No menu items found. Add dishes in Menu Manager!</div>';
      return;
    }
  ...
  orderForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok || res.status === 201) {
        ...
        showToast(
          `Order failed: ${data.details?.customer_phone || data.error || 'Check input'}`,
          'error',
        );
      }
    } catch (err) {
  ...
  function renderRecentChips() {
    if (!recentOrders.length) {
      recentChipsContainer.innerHTML =
        '<span class="text-muted text-sm">No recent orders yet</span>';
      return;
    }
    recentChipsContainer.innerHTML = '';
  ...
  function renderAdminMenu() {
    if (!menuCatalog.length) {
      adminMenuList.innerHTML =
        '<div class="text-muted text-sm py-4">No menu items found.</div>';
      return;
    }
