...
const ACTIONS = [
  ...
  // NEW ON DAY 3. Kept at a low weight on purpose: it blocks the Node event
  // loop, so a high weight would make the whole service look slow and hide the
  // very contrast we are trying to show.
  { weight: 4,  name: 'pairings',       run: () => hit('GET', `/teas/${rand(1, 5)}/pairings`) },
