 ...
 *
 * 3. REQUEST CORRELATION AND TRACE CORRELATION
 ...
 *
 *    NEW ON DAY 3: the same child logger also carries `trace_id`. That single
 *    field is the bridge between the three pillars -- it lets Grafana turn a
 *    log line into a clickable link straight into the trace in Jaeger. See
 *    grafana/provisioning/datasources/datasources.yml for the other half of
 *    the wiring.
 */
 ...
 * NEW ON DAY 3.
 *
 * Reads the trace and span id of whatever span is currently active. Returns an
 * empty object outside a traced operation, or when this trace was not sampled.
 *
 * `trace.getActiveSpan()` reads from OpenTelemetry's own context propagation,
 * which is built on the same AsyncLocalStorage mechanism we use for requestId.
 * Same idea, one layer down.
 *
 * Note the snake_case field names. They are not a style slip: `trace_id` and
 * `span_id` are the conventional names that Grafana, Loki and every tracing
 * backend expect to find in a log line.
 */
