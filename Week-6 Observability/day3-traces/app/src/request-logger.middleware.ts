...
import { rootLogger, requestContext, log, traceFields } from './logger';
...
export class RequestLoggerMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    ...
    // NEW ON DAY 3: trace_id joins requestId on the child logger.
    //
    // This runs inside the HTTP span that auto-instrumentation created, so
    // there IS an active span here and we can read its trace id. Every log
    // line for this request now carries it.
    //
    // Why both ids? requestId is ours and exists whether or not tracing is on
    // or the trace was sampled. trace_id is the tracing system's, and is the
    // one Jaeger can look up. In practice you keep both.
    const store = {
      requestId,
      logger: rootLogger.child({ requestId, ...traceFields() }),
    };
